function applyProfileData(data) {
    ...
    if (nameDisplay) {
        ...
        if (data.nameNeon) {
            ...
            nameDisplay.style.textShadow = `0 0 ${size}px ${color}, 0 0 ${size*2}px ${color}, 0 0 ${size*3}px ${color}`;
        }
        if (data.fontFamily) {
    ...
    if (emojiLeft) {
        ...
        emojiLeft.classList.toggle('d3', data.emoji3d !== false);
    }
    const emojiRight = document.getElementById('nameEmojiRight');
    if (emojiRight) {
        emojiRight.textContent = data.emojiRight || '';
        emojiRight.classList.toggle('d3', data.emoji3d !== false);
    }
